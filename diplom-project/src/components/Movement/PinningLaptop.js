...
export default function PinningLaptop() {
    ...
    useEffect(() =>{
        if(date === '' || laptop === '' || cabinet === '' || reasons === '') {
            setValid(false)
        }
        else setValid(true)
    }, [date, laptop, cabinet, reasons])
    ...
    useEffect(() => {
        const FetchData = async () => {
            let arr = [];
            await Axios.get('http://localhost:3001/laptop_movement').then((res) => {
                let result = res.data;
                result.map((laptop) => {
                    return arr.push({value: laptop.model, label: laptop.model, key: laptop.location, keys: laptop.laptop_id})
                })
                setValue(arr)
            })
        }
    ...
    const pinningLaptop = () => {
        ...
        Axios.post("http://localhost:3001/location_laptop", {
            ...
            if(response.data.message === 'Успешное добавление'){
                ...
                if(cabinet === 'Склад') {
                    Axios.post('http://localhost:3001/status_laptop', {
        ...
        <form onSubmit={FormSubmit}>
            <div style={{height: '190px'}}>
                <div style={{ width: '45%', float: 'left', height: '190px', padding: '5px'}}>
                    ...
                    <Label>Выберите ноутбук для перемещения:</Label>
                    <SELECT options={value} placeholder='Ноутбук...' onChange={(e) => {setLaptop(e.value); setKey(e.key); setId(+e.keys)}}></SELECT>
                </div>
                ...
                <Button disabled={!valid} isActive={valid} onClick={pinningLaptop}>Оформить</Button>
                <ToastContainer />
                </div>
        </form>
        </>
