...
export default function PinningFurniture() {
    ...
    useEffect(() => {
        const FetchData = async () => {
            let arr = [];
            await Axios.get('http://localhost:3001/furniture_movement').then((res) => {
                let result = res.data;
                result.map((furniture) => {
                    return arr.push({value: furniture.name + ' ' + furniture.model, label: furniture.name + ' ' + furniture.model, key: furniture.location, keys: furniture.furniture_id})
                })
                setValue(arr)
            })
        }
        ...
        <form onSubmit={FormSubmit}>
            <div style={{height: '190px'}}>
                <div style={{ width: '45%', float: 'left', height: '190px', padding: '5px'}}>
                    ...
                    <SELECT options={value} placeholder='Мебель...' onChange={(e) => {setFurniture(e.value); setKey(e.key); setId(+e.keys)}}></SELECT>
                </div>
                ...
                <Button disabled={!valid} isActive={valid} onClick={pinningFurniture}>Оформить</Button>
                <ToastContainer />
                </div>
        </form>
        </>
