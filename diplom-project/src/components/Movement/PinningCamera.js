    ...
    useEffect(() => {
        const FetchData = async () => {
            ...
            await Axios.get('http://localhost:3001/camera_movement').then((res) => {
                let result = res.data;
                result.map((camera) => {
                    return arr.push({value: camera.model, label: camera.model, key: camera.location, keys: camera.camera_id})
                })
                setValue(arr)
            })
        }
        ...
        <form onSubmit={FormSubmit}>
            <div style={{height: '190px'}}>
                <div style={{ width: '45%', float: 'left', height: '190px', padding: '5px'}}>
                    ...
                    <SELECT options={value} placeholder='Камера...' onChange={(e) => {setCamera(e.value); setKey(e.key); setId(+e.keys)}}></SELECT>
                </div>
                ...
                <Button disabled={!valid} isActive={valid} onClick={pinningCamera}>Оформить</Button>
                <ToastContainer />
                </div>
        </form>
        </>
