...
export default function Repair() {
  ...
  const FetchData = async () => {
    try {
      ...
      const ven = await Axios(
        'http://localhost:3001/select_repair_ventilation',
      );
        ...
        >
          <nav className="nav-repair">
            <ul className="ul-repair">
              <li
                className={`li-repair ${type === 'Все' ? 'active' : ''}`}
                onClick={() => setType('Все')}
              >
                Все записи
              </li>
              <li
                className={`li-repair ${type === 'Компьютер' ? 'active' : ''}`}
                onClick={() => setType('Компьютер')}
              >
                Компьютер
              </li>
              <li
                className={`li-repair ${type === 'Ноутбук' ? 'active' : ''}`}
                onClick={() => setType('Ноутбук')}
              >
                Ноутбук
              </li>
              <li
                className={`li-repair ${type === 'Монитор' ? 'active' : ''}`}
                onClick={() => setType('Монитор')}
              >
                Монитор
              </li>
              <li
                className={`li-repair ${type === 'МФУ' ? 'active' : ''}`}
                onClick={() => setType('МФУ')}
              >
                МФУ
              </li>
              <li
                className={`li-repair ${type === 'Камера' ? 'active' : ''}`}
                onClick={() => setType('Камера')}
              >
                Камера
              </li>
              <li
                className={`li-repair ${type === 'Мебель' ? 'active' : ''}`}
                onClick={() => setType('Мебель')}
              >
                Мебель
              </li>
              <li
                className={`li-repair ${type === 'Система' ? 'active' : ''}`}
                onClick={() => setType('Система')}
              >
                Система вентиляции
              </li>
            </ul>
          ...
          {type === 'Компьютер' && (
            <RepairTable repair={computer} type={'Компьютер'} />
          )}
          {type === 'Ноутбук' && (
            <RepairTable repair={laptop} type={'Ноутбук'} />
          )}
          {type === 'Монитор' && (
            <RepairTable repair={screen} type={'Монитор'} />
          )}
          {type === 'МФУ' && <RepairTable repair={scanner} type={'МФУ'} />}
          {type === 'Камера' && <RepairTable repair={camera} type={'Камера'} />}
          {type === 'Мебель' && (
            <RepairTable repair={furniture} type={'Мебель'} />
          )}
          {type === 'Система' && (
            <RepairTable repair={ventilation} type={'Система вентиляции'} />
          )}
        </div>
      </div>
    </>
