...
export default function FurnitureSelection() {
    const [furniture, setFurniture] = useState({name: '', model: '', price: ''})
    const [errors, setErrors] = useState({})
    const [addStatus, setAddStatus] = useState('');
      
      const handleFurnitureChange = (event) => {
        setFurniture(furniture => ({...furniture, [event.target.name] : event.target.value}));
    ...
    return (
        <form style={{width: '420px'}} onSubmit={FormSubmit}>
            ...
            <input type="text" id="form-input" name="name" value={furniture.name} style={{borderColor: errors.name ? 'red' : null}} 
            onChange={handleFurnitureChange}/>
        {(errors.name) && <span className='err'>{errors.name}</span>}
            <label htmlFor="model" className='add'>Модель:</label>
            <input type="text" id="form-input" name="model" value={furniture.model} style={{borderColor: errors.model ? 'red' : null}} 
            onChange={handleFurnitureChange}/>
        {(errors.model) && <span className='err'>{errors.model}</span>}
            <label htmlFor="price" className='add'>Цена:</label>
            <input type="text" id="form-input" name="price" value={furniture.price} style={{borderColor: errors.price ? 'red' : null}} 
            onChange={handleFurnitureChange}/>
