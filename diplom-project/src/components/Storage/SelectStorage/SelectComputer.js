...
import { faUserLock, faHouseLock, faTrash, faScrewdriverWrench, faRotateRight } from '@fortawesome/free-solid-svg-icons';
import Button from "../../Button/Button";

export default function SelectComputer() {
    const navigate = useNavigate('')
    const [computer, setComputer] = useState([])
    const [now, setNow] = useState({name: '', id: '', videocard : '', videocard_id: '', processor: '', processor_id: '', card: '', card_id: '', memory: '', memory_id: '', disk: '', disk_id: ''})
    ...
    const ViewComputer = (id) => {
        Axios.get(`http://localhost:3001/computer/${id}`).then((res) => {
            let result = res.data
            setNow({name: result[0].name, id: result[0].id_computer, videocard: result[0].videocards, videocard_id: result[0].videocard_id,
                processor: result[0].processors, processor_id: result[0].processor_id, card: result[0].mothercards, card_id: result[0].mothercard_id, 
                memory: result[0].memories, memory_id: result[0].memory_id,disk: result[0].disks, disk_id: result[0].disk_id
            })
            console.log(result)
        })
    }
            ...
            <div className="util-modal">
                <div className="util-content">
                    ...
                    <div style={{marginTop: '1rem', width: '660px', padding: '22px'}}>
                        ...
                        <label className="add">Наименование утилизируемого компьютера</label>
                ...
                <div className="util-modal">
                    <div className="repair-content">
                        ...
                        <div style={{width: '660px', padding: '22px'}}>
                            ...
                            <Button isActive style={{float: 'left'}} onClick={RepairComputer}>Отправить в ремонт</Button>
                            <Button isActive style={{float: 'right'}} onClick={() => setActive(false)}>Назад</Button>
                        </div>
                    </div>
                </div>
    ...
    const ReplaceMenu = ({value, type, object, change, click}) => {
        ...
        return (
            ...
            <option>...</option>
            {type === 'Видеокарта' && object.map((item, i) => {return (<option key={i} value={[item.id_videocard, item.model]}>{item.model}</option>)})}
            {type === 'Процессор' && object.map((item, i) => {return (<option key={i} value={[item.id_processor, item.model]}>{item.model}</option>)})}
            {type === 'Материнская плата' && object.map((item, i) => {return (<option key={i} value={[item.id_mothercard, item.model]}>{item.model}</option>)})}
            {type === 'Оперативная память' && object.map((item, i) => {return (<option key={i} value={[item.id_memory, item.model]}>{item.model}</option>)})}
            {type === 'Жесткий диск' && object.map((item, i) => {return (<option key={i} value={[item.id_disk, item.model]}>{item.model}</option>)})}
            </select>
            <Button isActive onClick={click}>Осуществить замену</Button>
            </>
        )
    }
        ...
        return(
            <div className="util-modal">
                ...
                <div style={{width: '', padding: '22px'}}>
                    <label className="add">Выберите элемент компьютера для замены:</label>
                    ...
                    {(type === 'Видеокарта') && <ReplaceMenu value={now.videocard} type={'Видеокарта'} object={videocard} change={(e) => setObject(e.target.value)} click={ChangeVideocard}/>}
                    {(type === 'Процессор') && <ReplaceMenu value={now.processor} type={'Процессор'} object={processor} change={(e) => setObject(e.target.value)} click={ChangeProcessor}/>}
                    {(type === 'Материнская плата') && <ReplaceMenu value={now.card} type={'Материнская плата'} object={card} change={(e) => setObject(e.target.value)} click={ChangeMothercard}/>}
                    {(type === 'Оперативная память') && <ReplaceMenu value={now.memory} type={'Оперативная память'} object={memory} change={(e) => setObject(e.target.value)} click={ChangeMemory}/>}
                    {(type === 'Жесткий диск') && <ReplaceMenu value={now.disk} type={'Жесткий диск'} object={disk} change={(e) => setObject(e.target.value)} click={ChangeDisk}/>}
                    <Button style={{marginTop: '3rem'}} isActive onClick={() => setActive(false)}>Назад</Button>
                </div>
                </div>
            </div>
        )
                            ...
                            computer.map((computer, i) =>{
                                return(
                                    <tr key={i}>
                                        ...
                                        <td>
                                            <button className="util" onClick={() => {setModal(true) ;ViewComputer(computer.id_computer)}}><FontAwesomeIcon style={{color: 'fff', cursor: 'pointer', marginRight: '7px'}} icon={faTrash}/>Утилизация</button>
                                            <button className='repair' onClick={() => {setRepair(true) ;ViewComputer(computer.id_computer)}}> <FontAwesomeIcon style={{color: 'fff', cursor: 'pointer', marginRight: '7px'}} icon={faScrewdriverWrench}/>Ремонт</button>    
                                            <button className="change" onClick={() => {setReplace(true); ViewComputer(computer.id_computer)}}><FontAwesomeIcon style={{color: '000', cursor: 'pointer', marginRight: '7px'}} icon={faRotateRight}/>Замена</button>
                                        </td>
                                    </tr>
                                )
                            })
