...
export default function ScannerSelection() {
    const [scanner, setScanner] = useState({model: '', color: '', speed: '', price: ''})
    const [errors, setErrors] = useState({})
    const [addStatus, setAddStatus] = useState('');

    const handleScannerChange = (event) => {
        setScanner(scanner => ({...scanner, [event.target.name] : event.target.value}));
        console.log(scanner)
    }
    ...
    return (
        <form style={{width: '420px'}} onSubmit={FormSubmit}>
            ...
            <input type="text" id="form-input" name="model" value={scanner.model} style={{borderColor: errors.model ? 'red' : null}} 
            onChange={handleScannerChange}/>
            
            {(errors.model) && <span className='err'>{errors.model}</span>}
            
            <label htmlFor="color" className='add'>Цвет печати:</label>
            <select id="form-input" name="color" value={scanner.color} onChange={handleScannerChange}>
                <option>...</option>
                <option>Черно-белая</option>
                <option>Цветная</option>
                <option>Оба</option>
            </select>
            <label htmlFor="speed" className='add'>Скорость печати (стр/мин):</label>
            <input type="text" name="speed" id="form-input" value={scanner.speed} style={{borderColor: errors.speed ? 'red' : null}} 
            onChange={handleScannerChange}/>
            
            {(errors.speed) && <span className='err'>{errors.speed}</span>}
            
            <label htmlFor="price" className='add'>Цена:</label>
            <input type="text" id="form-input" name="price" value={scanner.price} style={{borderColor: errors.price ? 'red' : null}} 
            onChange={handleScannerChange}/>
