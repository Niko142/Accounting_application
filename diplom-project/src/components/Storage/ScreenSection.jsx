...
export default function ScreenSection() {
  ...
  return (
    <form style={{ width: '420px' }} onSubmit={FormSubmit}>
      ...
      <label htmlFor="diagonal" className="add">
        Диагональ (в дюймах):
      </label>
      <input
        type="text"
        ...
        style={{ borderColor: errors.diagonal ? 'red' : null }}
        onChange={handleScreenChange}
      />

      {errors.diagonal && <span className="err">{errors.diagonal}</span>}

      <label htmlFor="rate" className="add">
        Частота (в формате `число` ГЦ):
      </label>
      <input
        type="text"
        name="rate"
      ...
      {addStatus === false && (
        <Fail>Возникла ошибка при добавлении монитора</Fail>
      )}
    </form>
  );
}
