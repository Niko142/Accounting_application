...
export default function AddComponents() {
    ...
    useEffect(() => {
        if(hasErrorModelProcessor || hasErrorRateProcessor || hasErrorPriceProcessor) {
            setValidProcessor(false)
        } 
        else {setValidProcessor(true)}
    }, [validProcessor, hasErrorModelProcessor, hasErrorRateProcessor, hasErrorPriceProcessor])
    ...
    useEffect(() => {
        ...
        else {setValidMemory(true)}
    }, [validMemory, hasErrorModelMemory, hasErrorVolumeMemory, hasErrorPriceMemory])
    ...
    return(
        ...
        <Button id='image-button' isActive onClick={() => navigate('/components')}>Назад</Button>
        <Button id='image-button' isActive onClick={() => setCategory('')}>Сбросить категорию</Button>
        </section>
        <div className="block-select">
            <select id="category" className="cont" value={category}
            onChange={(e) => setCategory(e.target.value)}>{components.map(item => {return (<option key={item.value}>{item.name}</option>)})}</select>  
        </div>
        
        {(category === 'Видеокарта') && 
        <>
        <div className="block-input">
            <Vitrina title={'Форма для добавления видеокарты'} src={process.env.PUBLIC_URL + '/triple.png'} width={'160'} alt={'Видеокарта'}/>
            <form style={{display: 'block', width: '460px', height: '190px'}} onSubmit={FormSubmit}>
                <input type="text" id="components-input" placeholder="Модель" value={modelVideocard} style={{borderColor: hasErrorModelVideo ? 'red' : null}}  
                onChange={(event) => {setModelVideocard(event.target.value); setHasModelVideo(event.target.value.trim().length === 0)}}/>
                {(hasErrorModelVideo) && <span id = 'err'> Поле 'Модель' пусто</span>}
                <input type="text" id="components-input" placeholder="Стоимость" value={priceVideocard} style={{borderColor: hasErrorModelVideo ? 'red' : null}}
                onChange={handlePriceChange}/>
                {(hasErrorPriceVideo) && <span id="err">Поле 'Цена' пусто</span>}
                <Button disabled={!validVideocard} isActive={validVideocard} style={{float: 'right'}} onClick = {addVideocard}>Добавить</Button>
                <ToastContainer />
            </form>
        </div></>}
        {(category === 'Процессор') &&
        <>
        <div className="block-input">
            <Vitrina title={'Форма для добавления процессора'} src={process.env.PUBLIC_URL + '/processor.png'} width={'160'} alt={'Процессор'}/>
            <form style={{display: 'block', width: '470px', height: '270px'}} onSubmit={FormSubmit}>
                <input type="text" id="components-input" value={modelProcessor} placeholder="Модель" 
                style={{borderColor: hasErrorModelProcessor ? 'red' : null}} 
                onChange={(event) => {setModelProcessor(event.target.value); setHasModelProcessor(event.target.value.trim().length === 0)}}/>
                {(hasErrorModelProcessor) && <span id = 'err'> Поле 'Модель' пусто</span>}
                <input type="text" id="components-input" value={rateProcessor} style={{borderColor: hasErrorRateProcessor ? 'red' : null}} placeholder="Частота (в формате, 'x.x ГГц')" 
                onChange={handleRateProcessorChange}/>
                {(hasErrorRateProcessor) && <span id = 'err'>{hasErrorRateProcessor}</span>}
                <input type="text" id="components-input" value={priceProcessor} style={{borderColor: hasErrorPriceProcessor ? 'red' : null}} placeholder="Стоимость" 
                onChange={handlePriceChange}/>
                {(hasErrorPriceProcessor) && <span id = 'err'> Поле 'Цена' пусто</span>}
                <Button disabled={!validProcessor} isActive={validProcessor} style={{float: 'right'}} onClick={addProcessor}>Добавить</Button>
                <ToastContainer />
            </form>
        </div>
        </>}
        ...
        <>
            <div className="block-input">
            <Vitrina title={'Форма для добавления материнской платы'} src={process.env.PUBLIC_URL + '/mothercard.png'} width={'170'} alt={'Материнская плата'}/>
            <form style={{display: 'block', width: '570px', height: '310px'}} onSubmit={FormSubmit}>
                <input type="text" id="components-input" placeholder="Модель" value={modelCard} style={{borderColor: hasErrorModelCard ? 'red' : null}} 
                onChange={(e) => {setModelCard(e.target.value); setHasModelCard(e.target.value.trim().length === 0)}}/>
                {(hasErrorModelCard) && <span id="err">Поле "Модель" пусто</span>}
                <select id="components-input" value={typeCard} onChange={(e) => setTypeCard(e.target.value)}>
                ...
                <input type="text" id="components-input" placeholder="Частота (в формате 'xxxx МГц')" value={rateCard} style={{borderColor: hasErrorRateCard ? 'red' : null}} 
                onChange={handleRateCardChange}/>
                {(hasErrorRateCard) && <span id="err">{hasErrorRateCard}</span>}
                <input type="text" id="components-input" value={priceCard} placeholder="Стоимость" style={{borderColor: hasErrorPriceCard ? 'red' : null}} 
                onChange={handlePriceChange}/>
                {(hasErrorPriceCard) && <span id="err">Поле "Цена" пусто</span>}
                <Button disabled={!validCard} isActive={validCard} style={{float: 'right'}} onClick = {addCard}>Добавить</Button>
                <ToastContainer />
            </form>
        </div>
        </>
        ...
        <>
            <div className="block-input">
            <Vitrina title={'Форма для добавления ОЗУ'} src={process.env.PUBLIC_URL + '/memory.png'} width={'180'} alt={'ОЗУ'}/>
            <form style={{display: 'block', width: '500px', height: '300px'}} onSubmit={FormSubmit}>
                <input type="text" id="components-input" value={modelMemory} placeholder="Модель" style={{borderColor: hasErrorModelMemory ? 'red' : null}} 
                onChange={(e) => {setModelMemory(e.target.value); setHasModelMemory(e.target.value.trim().length === 0)}}/>
                {(hasErrorModelMemory) && <span id="err">Поле "Модель" пусто</span>}
                <select id="components-input" value={typeMemory} onChange={(e) => setTypeMemory(e.target.value)}>
                    <option>Выберите тип поддерживаемой памяти...</option>
                    <option>DDR2</option>
                    <option>DDR3</option>
                    <option>DDR4</option>
                ...
                <input type="text" id="components-input" value={volumeMemory} style={{borderColor: hasErrorVolumeMemory ? 'red' : null}}  placeholder="Объем памяти" 
                onChange={handleVolumeMemoryChange}/>
                {(hasErrorVolumeMemory) && <span id="err">{hasErrorVolumeMemory}</span>}
                <input type="text" id="components-input" placeholder="Стоимость" style={{borderColor: hasErrorPriceMemory ? 'red' : null}}  value={priceMemory} 
                onChange={handlePriceChange}/>
                {(hasErrorPriceMemory) && <span id="err">Поле "Цена" пусто</span>}
                <Button disabled={!validMemory} isActive={validMemory} style={{float: 'right'}} onClick={addMemory}>Добавить</Button>
                <ToastContainer />
            </form>
        </div>
        </>}
        ...
        <>
            <div className="block-input">
            <Vitrina title={'Форма для добавления диска'} src={process.env.PUBLIC_URL + '/disk.png'} width={'240'} alt={'Жесткий диск'}/>
            <form style={{display: 'block', width: '450px', height: '240px'}} onSubmit={FormSubmit}>
                <input type="text" id="components-input" value={modelDisk} placeholder="Модель" style={{borderColor: hasErrorModelDisk ? 'red' : null}} 
                onChange={(e) => {setModelDisk(e.target.value); setHasErrorModelDisk(e.target.value.trim().length === 0)}}/>
                {(hasErrorModelDisk) && <span id="err">Поле "Модель" пусто</span>}
                <input type="text" id="components-input" value={volumeDisk} placeholder="Объем" style={{borderColor: hasErrorVolumeDisk ? 'red' : null}}
                onChange={handleVolumeDiskChange}/>
                {(hasErrorVolumeDisk) && <span id="err">{hasErrorVolumeDisk}</span>}
                <input type="text" id="components-input" value={priceDisk} placeholder="Стоимость" style={{borderColor: hasErrorPriceDisk ? 'red' : null}}
                onChange={handlePriceChange}/>
                {(hasErrorPriceDisk) && <span id="err">Поле "Цена" пусто</span>}
                <Button disabled={!validDisk} isActive={validDisk} style={{float: 'right'}} onClick={addDisk}>Добавить</Button>
                <ToastContainer />
            </form>
        </div>
        </>
