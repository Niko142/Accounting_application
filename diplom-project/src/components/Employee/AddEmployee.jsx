      ...
      <form className="section_employee" onSubmit={FormSubmit}>
        <div className="add_employee">
          ...
          <input
            ...
            id="form-input"
            name="surname"
            style={{ borderColor: errors.surname ? 'red' : null }}
          ...
          <input
            ...
            id="form-input"
            name="patronymic"
            style={{ borderColor: errors.patronymic ? 'red' : null }}
            value={employee.patronymic}
            onChange={handleEmployeeChange}
          />
          {errors.patronymic && (
            <span className="err">{errors.patronymic}</span>
          )}
          ...
          {phoneReg && isPossiblePhoneNumber(phoneReg) ? (
            <div style={{ color: 'green' }}>
              Номер телефона введен корректно
            </div>
          ) : (
            <div style={{ color: 'red' }}>Неправильный формат номера</div>
          )}
          <section style={{ marginLeft: '56px', marginTop: '1rem' }}>
            <Button isActive onClick={addEmployee}>
              Добавить
            </Button>
